...
```
Nếu **bất kỳ** check nào fail thì fix trước khi bắt đầu lab. Đừng cố start lab khi chưa có RAG pipeline — bạn sẽ stuck rất nhanh.
...
- **Phase A (60')**: RAGAS Eval (30 điểm)
- **Phase B (60')**: LLM-as-Judge (25 điểm)
- **Phase C (90')**: Guardrails (35 điểm)
- **Blueprint (30')**: Document (10 điểm)
...
**Pattern:** Questions cần kết hợp facts từ 2+ documents để trả lời.
**Examples:**
- "Compare X and Y across documents..."
- "What changed between version A and B..."
**Root cause:** Retriever chỉ lấy top-3 chunks, không đủ context cho multi-hop.
**Proposed fix:**
- Tăng `top_k` từ 3 -> 5
...
#### Acceptance criteria
- [ ] Work-flow file valid YAML (test với yamllint)
- [ ] Có threshold gate — exit code 1 nếu metric < target
- [ ] Có artifact upload cho audit/debugging

**Tip:** Bạn không cần thực sự push lên GitHub. File `.yml` đúng syntax + script Python tương ứng là đủ.
...
```python
    ...
    print("Substantial agreement – production-ready ✔️")
else:
...
Viết le `judge_bias_report.md` documenting **ít nhất 2 biases**:
...
test_inputs = [
    ...
    # Mixed
    "Customer Nguyễn Văn A, CCCD 0987654321, phone 0912345678",
    # Edge cases
    "",                                # Empty
...
for layer in ['L1', 'L2', 'L3']:
    ...
    print(f"{layer}: P50={np.percentile(vals, 50):.0f}ms, P95={np.percentile(vals, 95):.0f}ms")
```
...
Dễ ne ít nhất 5 SLOs với alert thresholds:
## SLOs
...
Dùng checklist này TRƯỚC khi submit. Nếu chưa check ≥ 80% items, có thể bạn chưa pass threshold 60 điểm.
